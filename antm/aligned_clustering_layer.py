...
def hdbscan_cluster(embedding, size) : ## Modified to return membership scores as well
    clusters_labels = []
    cluster_proba = []
    ...
    return clusters_labels,cluster_proba
...
def plot_alignment_no_show(df_tm,umap_embeddings_visualization,clusters_labels,path):
    tm = df_tm[["window_num", "cluster_num", "C"]]
    tm_copy = tm.copy()
    tm_copy.loc[:, "name"] = tm.apply(lambda row: str(row["window_num"]) + "-" + str(row["cluster_num"]) , axis=1)
    tm = tm_copy
